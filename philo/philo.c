...
int	main(int argc, char **argv)
{
	...
	if (!init_simulation_params(&simul, argc, argv))
		return (1);
	if (!init_simulation(&simul))
		return (simul_clean(&simul), 1);
	if (!init_simulation_threads(&simul))
		return (simul_join_threads(&simul), simul_clean(&simul), 1);
	while (!is_simulation_finished(&simul))
		usleep(5000);
	simul_join_threads(&simul);
	return (simul_clean(&simul), 0);
}
